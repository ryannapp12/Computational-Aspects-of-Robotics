...
def sample_points(arm, obstacles, sx, sy, gx, gy):
    ...
    """
    cnt = 0
    smple = np.array([[sx, sy]])
    while cnt < N_SAMPLE:
        th = np.random.uniform(-np.pi, np.pi, (1, 2))
        flg = detect_collision(arm, obstacles, th)
        if not flg:
            smple = np.append(smple, th, axis=0)
            cnt = cnt + 1
    smple = np.append(smple, [[gx, gy]], axis=0)
    return smple[:, 0], smple[:, 1]
...
def construct_prm(arm, obstacles, sample_x, sample_y):
    ...
    """
    dta = np.array([sample_x, sample_y]).T
    knn = NearestNeighbors(algorithm='auto',
                           metric=lambda a, b: torus_dist(a, b),
                           ).fit(dta)
    dist, idx = knn.kneighbors(X=dta, n_neighbors=N_KNN)
    idx_new = []
    for i in range(len(dist)):
        idxinner = idx[i, :]
        distinner = dist[i, :]
        innerlist = []
        for j in range(len(distinner)):
            if distinner[j] <= MAX_EDGE_LEN:
                # flg = False
                # th1 = np.linspace(sample_x[i], sample_x[idxinner[j]], 3)
                # th2 = np.linspace(sample_y[i], sample_y[idxinner[j]], 3)
                # for k in range(len(th1)):
                #     th1i, th2i = toroid_to_euclidean_points(th1[k], th2[k])
                #     pdb.set_trace()
                #     flgtmp = detect_collision(arm, obstacles, [th1i, th2i])
                #     if flgtmp:
                #         flg = True
                #         break
                # if not flg:
                #     innerlist.append(idxinner[j])
                innerlist.append(idxinner[j])
        idx_new.append(innerlist)
    return idx_new
